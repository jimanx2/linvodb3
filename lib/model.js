...
function Model (name, schema, options) {
  ...
  if (self.options.autoLoad) 
		self.initStore(self.options.dbInstance);

  return self;
};
Model.defaults = { dbInstance: null, autoIndexing: true, autoLoad: true };
 ...
 */
Model.prototype.initStore = function(dbInstance) {
  ...
  this.store = stores[path.resolve(filename)];
	if (this.store && this.store.isOpen()){
		this.store = stores[path.resolve(filename)] = this.store
	} else if (dbInstance) {
		this.store = dbInstance.sublevel(filename)
	} else {
		this.store = levelup(filename, this.options.store || { })
	}
